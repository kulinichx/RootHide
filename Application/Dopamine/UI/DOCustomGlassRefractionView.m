...
//
//  Keep the calibrated wallpaper + adaptive scrim as the one source of truth.
//  V2 uses a thin-lens edge profile, weak dispersion, isotropic diffusion and
//  restrained Fresnel/specular response so the center stays visually attached
//  to the real backdrop instead of reading as a magnifying sticker.
//
...
"    }\n"
"\n"
"    // Glass V2: keep the center close to backdrop identity and concentrate optical\n"
"    // displacement in a thin rounded rim. This reads as a glass sheet rather than\n"
"    // a convex magnifying lens.\n"
"    const float baseIOR = 1.45;\n"
"    const float opticalDepth = 2.20;\n"
"    const float dispersionDelta = 0.0015;\n"
"    float insideDepth = max(-d, 0.0);\n"
"    float2 center = u.viewSize * 0.5;\n"
"    float shortSide = max(min(u.viewSize.x, u.viewSize.y), 1.0);\n"
"    // V2.1B.1: keep the optical band proportional to compact CTA height. A fixed\n"
"    // bezel gets visually thick on short capsules, especially at the rounded ends.\n"
"    float adaptiveRimWidth = min(max(u.rimWidth, 0.001), max(2.5, shortSide * 0.10));\n"
"    float2 cornerCore = max(center - float2(radius), float2(0.0));\n"
"    float2 cornerDelta = max(abs(localPoint - center) - cornerCore, float2(0.0));\n"
"    float cornerBlend = smoothstep(0.0, max(radius * 0.42, 1.0),\n"
"                                   min(cornerDelta.x, cornerDelta.y));\n"
"    cornerBlend = cornerBlend * cornerBlend * (3.0 - 2.0 * cornerBlend);\n"
"    float rimWidth = max(adaptiveRimWidth * mix(1.0, 0.72, cornerBlend), 0.001);\n"
"    float rimT = clamp(insideDepth / rimWidth, 0.0, 1.0);\n"
"    float rimWeight = 1.0 - smoothstep(0.0, 1.0, rimT);\n"
"    float edgeWeight = pow(max(rimWeight, 0.0), 1.32);\n"
"    float2 outwardNormal = roundedBoxOutwardNormal(localPoint, u.viewSize, radius);\n"
"    float zoomAmount = clamp(0.80 / shortSide, 0.006, 0.014);\n"
"    float backdropZoom = 1.0 + zoomAmount;\n"
"    float2 bodyPoint = center + (localPoint - center) / backdropZoom;\n"
"    float profileSlope = 0.95 * pow(max(rimWeight, 0.0), 1.45);\n"
"\n"
...
"    }\n"
"\n"
"    float2 baseUV = aspectFillUV(u.wallpaperOrigin + localPoint, u);\n"
"    float3 baseColor = wallpaper.sample(linearSampler, baseUV).rgb;\n"
...
"    float3 refractedColor = float3(wallpaper.sample(linearSampler, uvR).r,\n"
"                                   wallpaper.sample(linearSampler, uvG).g,\n"
"                                   wallpaper.sample(linearSampler, uvB).b);\n"
"    float refractiveBlend = clamp(u.refractionAmount, 0.0, 1.0) *\n"
"                            (0.28 + 0.62 * edgeWeight);\n"
"    float3 color = mix(baseColor, refractedColor, refractiveBlend);\n"
"\n"
"    // Isotropic eight-tap diffusion avoids the cross-shaped blur signature of V1.\n"
"    float blurRadius = u.diffusionRadius * edgeWeight;\n"
"    if (blurRadius > 0.001) {\n"
...
"            float2( diagonal,-diagonal), float2(-diagonal,-diagonal)\n"
"        };\n"
"        float3 diffusion = float3(0.0);\n"
"        for (uint tap = 0; tap < 8; tap++) {\n"
"            float2 p = sourcePoint + taps[tap] * blurRadius;\n"
"            diffusion += wallpaper.sample(linearSampler,\n"
"                aspectFillUV(u.wallpaperOrigin + p, u)).rgb;\n"
"        }\n"
"        color = mix(color, diffusion * 0.125, 0.20 * edgeWeight);\n"
"    }\n"
"\n"
...
"\n"
"    // Thin edge reflection: a narrow directional glint plus restrained Fresnel.\n"
"    // The darker opposite edge is deliberately weaker than V1 to avoid a plastic rim.\n"
...
"    float ndh = max(dot(surfaceNormal, halfDir), 0.0);\n"
"    float tightSpec = pow(ndh, 58.0);\n"
"    float broadSpec = pow(ndh, 14.0);\n"
"    float edgeCore = exp(-pow(insideDepth / 0.62, 2.0));\n"
"    float edgeBody = exp(-pow(insideDepth / 2.45, 2.0));\n"
"    // Bright wallpaper already supplies a strong light field. Reduce white glare there\n"
"    // and let the dark contour carry just enough separation for the edge to remain legible.\n"
"    float backdropLuminance = dot(color, float3(0.2126, 0.7152, 0.0722));\n"
"    float brightBackdrop = smoothstep(0.42, 0.88, backdropLuminance);\n"
"    float highlightResponse = mix(1.0, 0.46, brightBackdrop);\n"
"    float contourResponse = mix(0.90, 1.12, brightBackdrop);\n"
"    float specular = u.specularStrength * edgeWeight *\n"
"                     (0.14 * fresnel + 0.48 * tightSpec + 0.08 * broadSpec) *\n"
"                     (0.30 + 0.70 * edgeBody) * highlightResponse;\n"
"    float2 lightXY = normalize(lightDir.xy);\n"
"    float opposing = max(dot(outwardNormal, -lightXY), 0.0);\n"
"    float darkThickness = u.darkEdgeStrength * edgeWeight * edgeBody *\n"
"                          (0.05 + 0.50 * opposing) * contourResponse;\n"
"    float farGlint = u.specularStrength * 0.055 * edgeCore * opposing * highlightResponse;\n"
"    color *= (1.0 - darkThickness);\n"
"    color += float3(specular + farGlint);\n"
"    color = clamp(color, float3(0.0), float3(1.0));\n"
"\n"
"    // V2.1B.1 narrows the overlay to the actual optical edge. Bright backgrounds get\n"
"    // a small additional alpha reduction so the rim never reads as a second acrylic strip.\n"
"    float overlayVisibility = mix(1.0, 0.78, brightBackdrop);\n"
"    float overlayAlpha = mask * pow(max(edgeWeight, 0.0), 1.38) *\n"
"                         clamp(u.edgeOverlayOpacity, 0.0, 1.0) * overlayVisibility;\n"
