...
// Both processes must be paused while the credential pointer is exchanged.
int proc_copy_ucred(uint64_t procCopyFrom, uint64_t procCopyTo)
{
	...
	if (!ucredToCopy || !origUcred) return ESRCH;

	// Hold the replacement before publishing it. Keep the target's original
	// credential reference alive until the pointer update has been verified, so
	// a failed write cannot leave the process referencing a prematurely released
	// credential.
	kauth_cred_ref(ucredToCopy);
	kauth_cred_hold(ucredToCopy);
	proc_ucred_update(procCopyTo, ucredToCopy);

	uint64_t observedUcred = proc_ucred(procCopyTo);
	if (observedUcred != ucredToCopy) {
		kauth_cred_drop(ucredToCopy);
		kauth_cred_unref(ucredToCopy);
		return EIO;
	}
...
int target_proc_with_ucred(const char *procPath, uid_t uid, gid_t gid, uid_t ruid, gid_t rgid, gid_t groups[NGROUPS_MAX])
{
	...
	for (int i = 0; i < NGROUPS_MAX; i++) snprintf(groupsStrings[i], sizeof(groupsStrings[i]), "%d", groups[i]);

	const char *argv[1 + 6 + (5 * 1) + (1 * NGROUPS_MAX) + 1];
	int idx = 0;
	argv[idx++] = procPath;
	argv[idx++] = "--fd";   argv[idx++] = "3";
	argv[idx++] = "--uid";  argv[idx++] = uidString;
	argv[idx++] = "--ruid"; argv[idx++] = ruidString;
	argv[idx++] = "--gid";  argv[idx++] = gidString;
	argv[idx++] = "--rgid"; argv[idx++] = rgidString;
	argv[idx++] = "--groups";
	...
	pid_t pid = 0;
	int spawnStatus = posix_spawn(&pid, procPath, &act, NULL, (char *const *)argv, (char *const *)envp);
	...
	// Never let a broken dyld-patch/donor path stall launchd indefinitely.
	struct pollfd readyPoll = {
		.fd = comPipe[0],
		.events = POLLIN,
	};
	int pollStatus;
	do {
		pollStatus = poll(&readyPoll, 1, 10000);
	} while (pollStatus < 0 && errno == EINTR);

	char ready = 0;
	ssize_t nread = -1;
	if (pollStatus > 0 && (readyPoll.revents & (POLLIN | POLLHUP))) {
		nread = read(comPipe[0], &ready, sizeof(ready));
	}
	close(comPipe[0]);
	if (pollStatus <= 0 || nread != sizeof(ready) || ready != 0x42) {
		kill(pid, SIGKILL);
